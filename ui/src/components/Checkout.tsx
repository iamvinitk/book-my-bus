...
export interface ICheckoutContext {
  ...
  cardInformation: CardInfo | undefined;
  setCardInformation: React.Dispatch<React.SetStateAction<CardInfo | undefined>>;
  ...
  const checkout = (): void => {
    ...
    axios
      .post(`${process.env.REACT_APP_API_ENDPOINT}checkout/`, data, {
        headers: {
          Authorization: `Token ${localStorage.getItem(CONSTANTS.TOKEN)}`,
        },
      })
      .then((res) => {
...
function Checkout(): JSX.Element {
  ...
  const [routeInformation, setRouteInformation] = React.useState(undefined);
  const [email, setEmail] = React.useState("");
  const [phone, setPhone] = React.useState("");
  const [cardInformation, setCardInformation] = React.useState<CardInfo | undefined>({});
  ...
  useEffect(() => {
    debugger;
    console.log(routeId, routeInformation, Boolean(routeId) && routeInformation === null)
    if (Boolean(routeId) && !routeInformation ) {
      axios
